...
import { Collisions, Constants, Entities, Geometry, Maps, Maths, Models, Tiled, Types } from '@hard2kill/gladiatorz-common';
import { creditBalance, logGameResult } from '@hard2kill/shared';
...
export class GameState extends Schema {
    ...
    // Players: single
    //
    playerAdd(id: string, name: string, odinsId: string = '') {
    ...
    private bulletUpdate(bulletId: number) {
        ...
        this.players.forEach((player) => {
            ...
            if (!player.isAlive) {
                const killer = this.players[bullet.playerId];
                const potWon = player.pot;
                console.log(`[POT] Before: killer=${killer.name} pot=${killer.pot}, killed=${player.name} pot=${player.pot}`);
                killer.pot += player.pot;
                player.pot = 0;
                console.log(`[POT] After: killer=${killer.name} pot=${killer.pot}, killed=${player.name} pot=${player.pot}`);
                ...
                this.onMessage({
                    ...
                    params: {
                        ...
                        killedName: player.name,
                        killerPlayerId: bullet.playerId,
                        killedPlayerId: player.playerId,
                        potWon,
                        kickPlayer: potWon > 0, // Kick if they lost their pot
                    },
                });
