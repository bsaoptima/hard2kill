...
export class WastelandMatchmakingRoom extends Room {
    ...
    onJoin(client: Client, options: { playerName: string; odinsId: string; betAmount?: number; currency?: Currency }) {
        ...
        console.log(`[Wasteland Matchmaking] Player joined queue: ${options.playerName} (bet: ${betAmount} ${currency})`);
    ...
    private tryMatchPlayers(newPlayer: WaitingPlayer) {
        ...
        if (opponent) {
            ...
            console.log(`[Wasteland Matchmaking] Matching ${newPlayer.playerName} vs ${opponent.playerName} (bet: ${newPlayer.betAmount} ${newPlayer.currency})`);
        ...
        if (player.currency === 'cash' && player.odinsId && supabase) {
            try {
                ...
                if (error) {
                    ...
                    console.log(`[Wasteland Matchmaking] Player ${player.playerName} has reached cash bot game limit (${count}/${MAX_BOT_GAMES}) - keeping in queue for real opponent`);
                    return;
                }

                console.log(`[Wasteland Matchmaking] Player ${player.playerName} cash bot games: ${count}/${MAX_BOT_GAMES}`);
            } catch (err) {
                console.error('[Wasteland Matchmaking] Error checking bot games:', err);
            }
        }

        console.log(`[Wasteland Matchmaking] Matching ${player.playerName} vs Bot (bet: ${player.betAmount} ${player.currency})`);
