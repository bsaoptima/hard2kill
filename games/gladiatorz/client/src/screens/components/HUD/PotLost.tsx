...
interface PotLostProps {
    amount: number | null;
    killerName: string | null;
}

export function PotLost({ amount, killerName }: PotLostProps): React.ReactElement | null {
    ...
    return (
        <View style={{ ...styles.container, opacity: animating ? 1 : 0 }}>
            <div style={styles.killText}>ELIMINATED BY</div>
            <div style={styles.killerName}>{killerName}</div>
            <div style={styles.rewardContainer}>
                <span style={styles.minusSign}>-</span>
                <span style={styles.amount}>${amount}</span>
            </div>
            <div style={styles.subtitle}>POT LOST</div>
        </View>
    );
}
