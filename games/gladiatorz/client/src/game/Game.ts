...
export class Game {
    ...
    private updateBullets = () => {
        for (const bullet of this.bulletsManager.getAll()) {
            ...
            // Visual collision with players (not me)
            for (const player of this.playersManager.getAll()) {
            ...
            // Visual collision with me
            if (bullet.active && this.me && this.me.canBulletHurt(bullet.playerId, bullet.team) &&
                Collisions.circleToCircle(bullet.body, this.me.body)) {
                bullet.active = false;
                this.spawnImpact(bullet.x, bullet.y);
            }

            // Visual collision with walls
            if (bullet.active && this.walls.collidesWithCircle(bullet.body, 'half')) {
                bullet.active = false;
                this.spawnImpact(bullet.x, bullet.y);
            }

            // Visual collision with map bounds
            if (bullet.active && this.map.isCircleOutside(bullet.body)) {
                bullet.active = false;
            }
        }
    };
    ...
    getStats = (): Stats => {
        ...
        return {
            ...
            playerPot: this.me ? this.me.pot : 0,
            players,
