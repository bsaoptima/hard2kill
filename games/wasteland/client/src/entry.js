...
import walkAnim from './assets/animations/walk.fbx'
import runAnim from './assets/animations/run_forward.fbx'

// Keep mutant for NPCs (currently disabled)
import mutant from './assets/animations/mutant.fbx'
import mutantIdleAnim from './assets/animations/mutant breathing idle.fbx'
import attackAnim from './assets/animations/Mutant Punch.fbx'
import mutantWalkAnim from './assets/animations/mutant walking.fbx'
import mutantRunAnim from './assets/animations/mutant run.fbx'
...
import KillReward from './entities/UI/KillReward'
import PotLost from './entities/UI/PotLost'
import DamageFlash from './entities/UI/DamageFlash'
...
import NetworkedPlayer from './entities/Player/NetworkedPlayer'
...
class FPSGameApp{
  ...
  async LoadAssets(){
    ...
    //Player Character
    promises.push(this.AddAsset(character, fbxLoader, "character"));
    ...
    //Mutant (for NPCs - currently disabled)
    promises.push(this.AddAsset(mutant, fbxLoader, "mutant"));
    promises.push(this.AddAsset(mutantIdleAnim, fbxLoader, "mutantIdleAnim"));
    ...
    //Extract character animations for multiplayer - just idle for now
    this.characterAnims = {};
    this.SetAnim('idle', this.assets['idleAnim']);

    //Extract mutant anims (for NPCs if needed)
    this.mutantAnims = {};
    this.mutantAnims['idle'] = this.assets['mutantIdleAnim'].animations[0];
    this.mutantAnims['attack'] = this.assets['attackAnim'].animations[0];
    this.mutantAnims['die'] = this.assets['dieAnim'].animations[0];
  ...
  EntitySetup(){
    ...
    // Network Entity - created first so others can reference it
    const networkEntity = new Entity();
    networkEntity.SetName('Network');
    networkEntity.AddComponent(new NetworkManager());
    networkEntity.AddComponent(new RemotePlayerManager(this.scene, this.camera, this.assets['character'], this.characterAnims, this.assets['ak47'].scene));
    this.entityManager.Add(networkEntity);
    ...
    playerEntity.AddComponent(new Weapon(this.camera, this.assets['ak47'].scene, this.assets['muzzleFlash'], this.physicsWorld, this.assets['ak47Shot'], this.listener, this.scene ));
    playerEntity.AddComponent(new PlayerHealth());
    playerEntity.AddComponent(new NetworkedPlayer());
    ...
    // Removed NPC mutants - multiplayer only
    // const npcLocations = [
    //   [10.8, 0.0, 22.0],
    // ];
    ...
    const killRewardEntity = new Entity();
    killRewardEntity.SetName("KillReward");
    killRewardEntity.AddComponent(new KillReward());
    this.entityManager.Add(killRewardEntity);

    const potLostEntity = new Entity();
    potLostEntity.SetName("PotLost");
    potLostEntity.AddComponent(new PotLost());
    this.entityManager.Add(potLostEntity);

    const damageFlashEntity = new Entity();
    damageFlashEntity.SetName("DamageFlash");
    damageFlashEntity.AddComponent(new DamageFlash());
    this.entityManager.Add(damageFlashEntity);
